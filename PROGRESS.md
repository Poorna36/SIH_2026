...
> **Status:** Done (All 10 T-SB tests pass — 35/35 — 2026-09-02).
...
- [x] `AnchorPoint` dataclass (id, src_x, src_y, feature_class, gradient_magnitude) implemented
- [x] `AnchorSet` dataclass with `.save()`, `.load()`, `.as_numpy()` implemented
- [x] `_extract_shi_tomasi_grid()` — Phase 1 baseline: Shi-Tomasi corners across uniform 8×8 grid cells
- [x] `_deduplicate_anchors()` — greedy min-spacing pruning, sorted by gradient magnitude descending
- [x] `extract_anchors()` entry point — validates min/max count, assigns sequential IDs
- [x] **Phase 2+ (stratified)**: morphological bucket detectors:
  - [x] `_extract_craters()` — crater rim/floor detection (HoughCircles)
  - [x] `_extract_ridges()` — ridge/scarp detection (morphological top-hat + NMS)
  - [x] `_extract_maria()` — flat-textured maria regions (inverse-gradient weighted sampling)
  - [x] `_extract_shadow_boundaries()` — solar terminator boundary detection
  - [x] `_extract_polar_terrain()` — high-incidence-angle polar regions (stripe-based sampling)

### 10.2 — Physical Transformation Engine (`src/synthetic/transforms.py`)
- [x] `TransformParams` dataclass (all geometric + photometric parameters, 3×3 matrix)
- [x] `build_transform_matrix()` — exact 3×3 homogeneous M via cv2.getRotationMatrix2D
- [x] `apply_transform()` — cv2.warpAffine with Lanczos/bicubic/bilinear
- [x] `transform_gt_points()` — exact floating-point GT coordinate mapping via M @ homogeneous coords
- [x] `apply_illumination_gamma()` — I_out = I_in^gamma (gamma ∈ [0.7, 1.4])
- [x] `apply_mtf_blur()` — Gaussian sigma ∈ [0.5, 1.5] for sensor MTF simulation
- [x] `apply_pushbroom_noise()` — per-column gain offsets for pushbroom vertical striping
- [x] `apply_shadow_extension()` — directional morphological shadow extension in solar azimuth direction
- [x] `generate_synthetic_pair()` — composite transform generator (6-step fixed order, all enabled transforms)
- [x] Excluded transformations documented in code and config: perspective warp, JPEG, salt-pepper, color jitter
- [x] Output clipped to [0,1] after geometric warp (prevents Lanczos Gibbs ringing from escaping range)

### 10.3 — Synthetic Benchmark Generator & Manifest (`scripts/generate_synthetic_benchmark.py`)
- [x] CLI: `--config configs/synthetic_benchmark.yaml --images data/raw/ --out data/synthetic/ --phase 1`
- [x] Phase 1 smoke test: single exact sub-pixel translation, 5 GT anchors, seed=42
- [x] Phase 2: N=10 seeds/condition, geometric transforms (scale + rotation + translation)
- [x] Phase 3: N=30 seeds/condition, photometric transforms added
- [x] Phase 4: N=50 seeds/condition, all transforms, all 6 terrain strata
- [x] Generates `synthetic_manifest.jsonl` (synthetic pair record per INTERFACES.md §11)
- [x] Hidden GT files saved to `data/synthetic/gt/<pair_id>_gt.json` (NEVER loaded by matcher)
- [x] Manifests split by benchmark phase and terrain stratum
- [x] Checkpointing: skips pairs if output exists (--force to override)
- [x] Leakage audit: verifies no GT file outside gt_dir (exit 4 on violation)
- [x] Exit codes 0–4 per PIPELINE.md §8

### 10.4 — Component-Wise Evaluation Engine (`src/evaluation/synthetic_eval.py`)
- [x] `GTAssignment` dataclass (gt_indices, pred_indices, distances, n_gt, n_pred, n_matched, n_fp, n_fn)
- [x] `assign_gt_predictions()` — 1-to-1 Hungarian matching (scipy.optimize.linear_sum_assignment) + greedy fallback
- [x] `score_l2_raw()` — GT Recall + Raw RMSE (before L3 filtering)
- [x] `score_l3_survival()` — GT Survival Rate + FP Pruning Rate
- [x] `score_l4_geometric()` — Inlier Precision, Inlier Recall, Pre-Refinement RMSE
- [x] `score_l5_refinement()` — Refinement Gain, % Improved, % Degraded, % < 1px, % < 0.5px
- [x] `score_l1_5_routing()` — MSM Routing Accuracy vs Oracle (no test data leakage enforced)
- [x] `compute_oracle_best_matcher()` — composite oracle score: argmax(0.5*(1/RMSE_norm) + 0.25*inlier_ratio + 0.25*coverage)
- [x] `aggregate_scorecards()` — N=50 seed aggregation, 95% CI (±1.96 SE or bootstrap)
- [x] `eval_synthetic` CLI runner — `scripts/eval_synthetic.py`

### 10.5 — Benchmark Orchestration & Reporting (`scripts/run_synthetic_benchmark.py`)
- [x] CLI: `--config --phase --matchers --out-csv results/synthetic_benchmark/synthetic_component_report.csv`
- [x] End-to-end blind pipeline runner: generates pairs → runs L1–L6 without accessing GT → evaluates all stages
- [x] Per-condition aggregation with 95% confidence intervals
- [x] Outputs:
  - [x] `results/synthetic_benchmark/synthetic_component_report.csv` — per-stage scorecard (L1.5, L2, L3, L4, L5 × matcher × condition)
  - [x] `results/synthetic_benchmark/synthetic_benchmark_summary.md` — narrative summary with statistical tables

### 10.6 — Unit & Regression Tests
- [x] T-SB01 — `transform_gt_points()` invertibility: applying M and M^-1 returns within 1e-9 px
- [x] T-SB02 — `extract_anchors()` grid coverage: at least one anchor per grid cell (8×8 = 64 cells)
- [x] T-SB03 — `extract_anchors()` minimum count gate: raises RuntimeError below min_count
- [x] T-SB04 — `assign_gt_predictions()` Hungarian: test with perfect predictions (all dist=0) → recall=1.0
- [x] T-SB05 — `assign_gt_predictions()` Hungarian: test with shifted predictions (all dist > max_dist_px) → recall=0.0
- [x] T-SB06 — `score_l5_refinement()` refinement gain: verify gain > 0 when refined_pts closer to GT
- [x] T-SB07 — `aggregate_scorecards()` CI: verify CI width decreases with N; CI includes true mean
- [x] T-SB08 — `generate_synthetic_pair()` photometric: output in [0, 1] range after all transforms
- [x] T-SB09 — `apply_illumination_gamma()`: verify gamma=1.0 returns identical image (identity)
- [x] T-SB10 — `compute_oracle_best_matcher()` oracle: verify matcher with RMSE=0 always wins
- [x] **35/35 tests pass** *(2026-09-02)*
...
| 10 | Synthetic GT Benchmark & Component-Wise Eval (v3.0) | **Done** (All 10 T-SB test suites pass — 35/35 tests, CLI orchestrator & eval scripts operational — 2026-09-02) |

> Updated 2026-09-01: Phase 7 marked In Progress for manual human annotation (`scripts/gt_annotator.py` tool ready).
> Updated 2026-09-02: Phase 10 completed — Synthetic Ground-Truth Benchmark Architecture v3.0 fully implemented and verified. Includes anchor extraction engine with morphological stratification, physical transformation engine, synthetic benchmark generator CLI (`scripts/generate_synthetic_benchmark.py`), component-wise evaluation engine (`src/evaluation/synthetic_eval.py` & `scripts/eval_synthetic.py`), end-to-end benchmark orchestrator (`scripts/run_synthetic_benchmark.py`), and 35/35 unit tests (T-SB01 through T-SB10) passing.
