...
export const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  return (
    <div className="w-full h-full overflow-y-auto sidebar-scroll p-6 md:p-10 space-y-8 bg-transparent text-white font-sans max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-white/10">
        <div>
          ...
          <p className="text-sm text-white/50 mt-2 flex items-center gap-3 flex-wrap font-sans">
            <span>Coordinates: <strong className="text-white font-medium">[{Math.abs(selectedScene.lat).toFixed(3)}°S, {Math.abs(selectedScene.lon).toFixed(3)}°E]</strong></span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={() => alert('Exporting PDS4 Ingestion Report...')}
      ...
      {/* ── HERO TELEMETRY STRIP (DIRECTLY STYLED AFTER HOME UI) ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 py-6 border-b border-white/10">
        <div className="space-y-1">
          <div className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight font-headline">
            {telemetry.rmsePx.toFixed(2)} px
          </div>
          <div className="text-xs text-white/50 font-medium">
            Registration RMSE (Sub-pixel goal met)
          </div>
        </div>
        ...
        <div className="space-y-1 sm:border-l sm:border-white/10 sm:pl-6">
          ...
          <div className="text-xs text-white/50 font-medium">
            3.0 µm Water-Ice Hydration
          </div>
        </div>

        <div className="space-y-1 lg:border-l lg:border-white/10 lg:pl-6">
          <div className="text-3xl sm:text-4xl font-extrabold text-emerald-400 tracking-tight font-headline">
            {slz.overallSafetyScore} / 100
          </div>
          <div className="text-xs text-white/50 font-medium">
            Safe Landing Score ({slz.goNoGo})
          </div>
        </div>

        <div className="space-y-1 border-l border-white/10 pl-6">
          <div className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight font-headline">
            {(telemetry.spatialCoverage * 100).toFixed(0)}%
          </div>
          <div className="text-xs text-white/50 font-medium">
            Homography Coverage ({telemetry.matcherWinner.toUpperCase()})
          </div>
        </div>
      </div>
