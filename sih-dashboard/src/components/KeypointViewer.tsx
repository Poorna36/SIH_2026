...
import { CheckCircle, AlertTriangle, Eye, MoveHorizontal, Grid3X3, Activity, Sparkles } from 'lucide-react';
...
export const KeypointViewer: React.FC<KeypointViewerProps> = ({ pairId = 'boguslawsky', keypoints: initialKeypoints, onProbeCoord }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#07080A] rounded-2xl overflow-hidden border border-white/10 shadow-2xl font-sans">
      {/* ── TOP TOOLBAR (ROUND PILL CONTROLS) ── */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-white/10 bg-black/60 backdrop-blur-md shrink-0">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#2997FF] shadow-[0_0_8px_rgba(41,151,255,0.8)]" />
            <span className="text-xs font-bold text-white tracking-wide">
              2D Co-Registration & Quality Control
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition-all cursor-pointer border ${
                showInliers
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition-all cursor-pointer border ${
                showOutliers
                  ? 'bg-red-500/20 border-red-500/50 text-red-300 shadow-[0_0_12px_rgba(239,68,68,0.3)]'
                  : 'bg-white/5 border-white/10 text-white/50'
              }`}
            >
            ...
            {viewMode === 'side-by-side' && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-white/10 hover:bg-white/20 border border-white/15 text-white transition-all cursor-pointer active:scale-95 shadow-sm"
                title="Replay Alignment Fusion Animation"
              >
                <Sparkles size={11} className="text-emerald-400 animate-spin" />
                <span className="hidden sm:inline">Replay Fusion Split</span>
              </button>
            )}
          </div>

          <div className="hidden lg:flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs font-semibold text-emerald-300">
              Verified: Same Crater (92.4% Match)
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center bg-white/5 border border-white/10 p-1 rounded-full">
          {[
            ...
            { id: 'split', label: 'Swipe Slider', icon: <MoveHorizontal size={12} /> },
      ...
      <div className="h-8 bg-black/80 backdrop-blur-md border-t border-white/10 px-4 flex items-center justify-between z-20 text-xs font-sans text-white/60 shrink-0">
        <div className="flex items-center gap-3">
          <span>Inliers: <strong className="text-white">{inlierCount}/40</strong></span>
          <span className="text-white/20">·</span>
          <span>RMSE: <strong className="text-[#2997FF]">0.34 px</strong></span>
          <span className="text-white/20">·</span>
